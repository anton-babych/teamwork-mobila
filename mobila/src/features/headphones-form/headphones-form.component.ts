...
import { CategoryType, Headphones } from 'entities/products/api';
  ...
  template: `
    <form class="form" [formGroup]="form" (ngSubmit)="onSubmit()">
      ...
      <div class="form-input">
        <label>smartphone id</label>
        <input type="text" [formControl]="smartphoneId" />
      </div>
...
export class HeadphonesFormComponent implements OnInit {
  @Input() item!: CategoryType;
  @Output() isSubmitted = new EventEmitter<{ product: Headphones; smartphoneId: string }>();

  form: FormGroup;
  smartphoneId = new FormControl('');
  ...
  ngOnInit() {
    ...
    this.form = new FormGroup<ControlsOf<Headphones>>({
  ...
  onSubmit() {
    this.isSubmitted.emit({
      product: this.form.value as Headphones,
      smartphoneId: this.smartphoneId.value as string,
    });
  }
}
